# 1600-Final-Project

# Introduction to Web APIs  
**Author:** Bennet  

## Summary  
This tutorial teaches beginners how to understand, interact with, and troubleshoot basic Web API requests. You will learn what APIs are, how they work, and how to use simple tools to make requests and view responses.  

## Target Audience  
This tutorial is designed for beginners with no programming experience, including high school students, entry-level college students, and anyone new to digital systems.

---

## Contents  
1. [What Are APIs?](intro_to_apis.md)  
2. [Making Your First API Request](making_requests.md)  
3. [Troubleshooting Common Issues](troubleshooting.md)

--- 
